...
interface UseOptimizedAuthFetchOptions {
  enabled?: boolean
...
interface UseOptimizedAuthFetchResult<T> {
  data: T | null
...
export function useOptimizedAuthFetch<T>(
  key: string,
  url: string,
  options: UseOptimizedAuthFetchOptions = {}
): UseOptimizedAuthFetchResult<T> {
  const {
    enabled = true,
